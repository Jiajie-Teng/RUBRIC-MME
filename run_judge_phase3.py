...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "logs" / "rubric_mme_phase3_official"
DEFAULT_MODEL = "gemini-3.1-pro-preview"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行 RUBRIC-MME 第三阶段官方 Gemini 裁判评分")
    parser.add_argument("--phase2-dir", default=str(DEFAULT_PHASE2_DIR), help="Phase 2 输出目录，需要包含 rounds.jsonl 和 dialogues.jsonl。")
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_DIR), help="Phase 3 输出目录。")
    parser.add_argument("--model", default=DEFAULT_MODEL, help="官方 judge 使用的模型名称。")
    parser.add_argument("--api-key-env", default="GOOGLE_API_KEY", help="官方 Gemini API key 所在的环境变量名。")
    ...
    parser.add_argument("--disable-response-schema", action="store_true", help="禁用 Gemini response_schema，改用 prompt-only JSON 模式。")
...
def main() -> None:
    ...
    def build_backend() -> OfficialGeminiJudgeBackend:
        return OfficialGeminiJudgeBackend(
            api_key_env=args.api_key_env,
            use_response_schema=not args.disable_response_schema,

...
DEFAULT_PHASE4_DIR = REPO_ROOT / "logs" / "rubric_mme_phase4_official"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "logs" / "rubric_mme_phase5_official"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行 RUBRIC-MME 第五阶段自动分析报告（官方 Gemini 接口版）")
    ...
    parser.add_argument("--api-key-env", default="GOOGLE_API_KEY", help="官方 Gemini API Key 的环境变量名")
    parser.add_argument("--use-response-schema", action="store_true", help="优先使用 Gemini 的 response_schema")
    parser.add_argument("--timeout-seconds", type=int, default=300, help="单次 Gemini 请求超时时间（秒）")
    parser.add_argument("--poll-interval", type=float, default=3.0, help="轮询 Gemini 结果的间隔秒数")

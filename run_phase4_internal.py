...
DEFAULT_PHASE3_DIR = REPO_ROOT / "logs" / "rubric_mme_phase3_internal"
DEFAULT_PHASE2_DIR = REPO_ROOT / "logs" / "rubric_mme_phase2_normalized"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "logs" / "rubric_mme_phase4_internal"
DEFAULT_MODEL = "gemini-3.1-pro-preview"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="运行 RUBRIC-MME 第四阶段内部版低分归因。")
    parser.add_argument("--phase3-dir", default=str(DEFAULT_PHASE3_DIR), help="Phase 3 输出目录，内部需要包含 turn_judgements.jsonl 和 session_judgements.jsonl。")
    ...
    parser.add_argument("--model", default=DEFAULT_MODEL, help="内部归因模型名称。")
    parser.add_argument("--api-url", default=DEFAULT_INTERNAL_API_URL, help="内部归因接口地址。")
    parser.add_argument("--api-key-env", default="MATRIXLLM_API_KEY", help="内部归因 token 所在环境变量名。")
    ...
    parser.add_argument("--top-p", type=float, default=0.95, help="归因模型 top_p。")
